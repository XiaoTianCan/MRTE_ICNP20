...
public final class RouterWaxman extends RouterModel {
    ...
    public RouterWaxman(int N,int HS, int LS, int nodePlacement,
			...
			int growthType, int bwDist, float bwMin, float bwMax) 
    {
	this.N = N;
	...
	if (growthType == ModelConstants.GT_ALL) {
		...
		while (numEdgesAdded < m || numNodesConnected < N) {
		    ...
		    // Node dst = (Node) g.getNodesVector().elementAt(rand2);    /*pick dst node randomly*/
		    ...
		    float u = (float)Distribution.getUniformRandom(ConnectRandom);
		    if (u < p){
			...
			if (dst.getOutDegree() == 1)
			    ++numNodesConnected;
			++numEdgesAdded;
			//Util.DEBUG("added edge:("+src.getID()+", "+dst.getID()+")  | numEdgesAdded="+numEdgesAdded+ " &numNodesConnected="+numNodesConnected);
		    }
		...
		while (numEdgesAdded  < m) {
		    ...
		    int rand = m+(int)  (Distribution.getUniformRandom(ConnectRandom)* (float) (N-m)); /*pick rand b/w m..N*/
		    Node dst = nodesV[rand];
		    if (i==rand) continue; 
		    if (g.hasEdge(src, dst)) continue;
